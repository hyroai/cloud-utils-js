...
const getSignedUrl = ({ blobService }) => (bucketName, fileName) => {
...
const downloadFileAsJson = ({ blobServiceClient }) => async (
  bucketName,
  fileName
) => {
  try {
...
const uploadJsonBlob = ({ blobServiceClient }) => (
  container,
  blobName,
  payload
) => {
...
const uploadFileBlob = ({ blobServiceClient }) => (
  container,
  blobName,
  filePath
) =>
